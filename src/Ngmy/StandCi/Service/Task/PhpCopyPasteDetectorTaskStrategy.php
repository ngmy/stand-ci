...
class PhpCopyPasteDetectorTaskStrategy implements TaskStrategyInterface {
	...
	public function process()
	{
		...
		$commands[] = $this->executable.' '.$this->extraArgs.' --log-pmd='.$this->outputDir.'/logs/phpcpd.xml app';
	...
	{
		$xml = new DomDocument;
		$xml->load($this->outputDir.'/logs/phpcpd.xml');

		$xsl = new DomDocument;
		$xsl->load(public_path('packages/ngmy/stand-ci/xsl/phpcpd.xsl'));

		$processor = new XSLTProcessor;
		$processor->importStyleSheet($xsl);

		File::put($this->outputDir.'/phpcpd.html', $processor->transformToXML($xml));
	}
